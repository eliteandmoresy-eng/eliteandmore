...
export default function NewBrandPage() {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        ...
        setBrandsCount(data.data?.length || 0);
        // Default sort order to last position
        setValue('sort_order', data.data?.length || 0);
      } catch {
        // Fallback
      }
    };
  ...
  const onSubmit = async (data: BrandFormData) => {
    try {
      ...
      toast.success('تم إضافة البرند بنجاح');
  ...
  const sortOptions = Array.from({ length: Math.max(brandsCount + 1, 20) }, (_, i) => ({
    label: `المركز ${i + 1}`,
    value: i.toString(),
  }));
  ...
  return (
    <div dir="rtl" className="font-tajawal max-w-4xl mx-auto pb-12 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold font-cairo text-elite-text">إضافة برند جديد</h1>
          <p className="text-elite-muted text-sm mt-1">أنشئ هوية بصرية ومعلومات لبرند جديد في متجرك</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-[32px] shadow-soft p-6 md:p-8 space-y-6 border border-white/50 backdrop-blur-sm">
            ...
            <Textarea
              label="الوصف"
              placeholder="اكتب وصفاً جذاباً عن البرند ومنتجاته..."
              rows={4}
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-elite-muted bg-surface-dim px-2 py-1 rounded-lg">PNG بخلفية محذوفة</span>
                </div>
                <Controller
                  name="logo_url"
                  control={control}
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-elite-muted bg-surface-dim px-2 py-1 rounded-lg">لتزيين صفحة البرند</span>
                </div>
                <Controller
                  name="cover_url"
                  control={control}
